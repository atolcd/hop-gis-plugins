package org.atolcd.hop.pipeline.transforms.gisgeoprocessing;
...
implements ITransform<GisGeoprocessingMeta, GisGeoprocessingData> {
    ...
    public GisGeoprocessing(TransformMeta s, GisGeoprocessingMeta meta, GisGeoprocessingData data, int c, PipelineMeta t, Pipeline dis) {
    ...
    private Geometry getOneGeometryGeoprocessing(String operator, Geometry inGeometry, Double distance) throws HopException {
    ...
    private Geometry getTwoGeometriesGeoprocessing(String operator, Geometry inGeometryA, Geometry inGeometryB, Double distance) throws HopException {
        ...
        if (!GeometryUtils.isNullOrEmptyGeometry(inGeometryA) && !GeometryUtils.isNullOrEmptyGeometry(inGeometryB)) {
            ...
            if (GeometryUtils.getSrid(inGeometryA).compareTo(GeometryUtils.getSrid(inGeometryB)) == 0) {
                	...
                	if(!GeometryUtils.isNullOrEmptyGeometry(inGeometryA)){
                		...
                		if(GeometryUtils.isNullOrEmptyGeometry(inGeometryB)){
		                    ...
		                    //Autres cas de découpe non pris en charge
		                    	...
		                    	throw new HopException("Split (MULTI)POLYGON by a (MULTI)LINESTRING, (MULTI)LINESTRING by (MULTI)LINESTRING or (MULTI)LINESTRING by (MULTI)POINT");
		                    }
                ...
                throw new HopException("Unauthorized mixed srids : " + GeometryUtils.getSrid(inGeometryA) + " with " + GeometryUtils.getSrid(inGeometryB));
            }
