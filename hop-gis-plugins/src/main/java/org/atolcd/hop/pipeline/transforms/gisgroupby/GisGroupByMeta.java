...
import org.atolcd.hop.pipeline.transforms.gisrelate.GisRelate;
...
public class GisGroupByMeta extends BaseTransformMeta implements ITransformMeta<GisGroupBy, GisGroupByData> {
    ...
    public static final String[] typeGroupLongDesc = { "-", BaseMessages.getString(PKG, "GroupByMeta.TypeGroupLongDesc.SUM"),
            ...
            BaseMessages.getString(PKG, "GroupByMeta.TypeGroupLongDesc.PERCENTILE"), BaseMessages.getString(PKG, "GroupByMeta.TypeGroupLongDesc.MIN"),
            BaseMessages.getString(PKG, "GroupByMeta.TypeGroupLongDesc.MAX"), BaseMessages.getString(PKG, "GroupByMeta.TypeGroupLongDesc.CONCAT_ALL"),
            BaseMessages.getString(PKG, "GroupByMeta.TypeGroupLongDesc.CONCAT_COMMA"), BaseMessages.getString(PKG, "GroupByMeta.TypeGroupLongDesc.FIRST"),
            BaseMessages.getString(PKG, "GroupByMeta.TypeGroupLongDesc.LAST"), BaseMessages.getString(PKG, "GroupByMeta.TypeGroupLongDesc.FIRST_INCL_NULL"),
            BaseMessages.getString(PKG, "GroupByMeta.TypeGroupLongDesc.LAST_INCL_NULL"), BaseMessages.getString(PKG, "GroupByMeta.TypeGroupLongDesc.CUMUMALTIVE_SUM"),
            BaseMessages.getString(PKG, "GroupByMeta.TypeGroupLongDesc.CUMUMALTIVE_AVERAGE"), BaseMessages.getString(PKG, "GroupByMeta.TypeGroupLongDesc.STANDARD_DEVIATION"),
            BaseMessages.getString(PKG, "GroupByMeta.TypeGroupLongDesc.CONCAT_STRING"), BaseMessages.getString(PKG, "GroupByMeta.TypeGroupLongDesc.COUNT_DISTINCT"),
    ...
    private void readData(Node stepnode) throws HopXmlException {
        try {
            ...
            throw new HopXmlException(BaseMessages.getString(PKG, "GroupByMeta.Exception.UnableToLoadStepInfoFromXML"), e);
        }
    }
    ...
    public void getFields(IRowMeta r, String origin, IRowMeta[] info, TransformMeta nextStep, IVariables space,
        ...
        for (int i = 0; i < subjectField.length; i++) {
            ...
            if (subj != null || aggregateType[i] == TYPE_GROUP_COUNT_ANY) {
                ...
                // cumulative average
                //
                if (aggregateType[i] == TYPE_GROUP_CUMULATIVE_AVERAGE && value_type == IValueMeta.TYPE_INTEGER) {
                    value_type = IValueMeta.TYPE_NUMBER;
                    precision = -1;
                    length = -1;
                    ...
                    precision = 0;
                } else if (aggregateType[i] == TYPE_GROUP_SUM && value_type != IValueMeta.TYPE_INTEGER && value_type != IValueMeta.TYPE_NUMBER
    ...
    public void check(List<ICheckResult> remarks, PipelineMeta transMeta, TransformMeta stepMeta, IRowMeta prev, String[] input, String[] output, IRowMeta info,
            IVariables space, IHopMetadataProvider metaStore) {
        CheckResult cr;

        if (input.length > 0) {
            cr = new CheckResult(ICheckResult.TYPE_RESULT_OK, BaseMessages.getString(PKG, "GroupByMeta.CheckResult.ReceivingInfoOK"), stepMeta);
            remarks.add(cr);
        } else {
            cr = new CheckResult(ICheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(PKG, "GroupByMeta.CheckResult.NoInputError"), stepMeta);
            remarks.add(cr);
        }
    }
	...
	@Override
	public ITransform createTransform(TransformMeta transformMeta, GisGroupByData data, int cnr, PipelineMeta pipelineMeta,
			Pipeline pipeline) {
