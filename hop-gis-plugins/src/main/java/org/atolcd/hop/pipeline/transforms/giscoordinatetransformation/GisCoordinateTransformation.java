...
import org.atolcd.hop.gis.utils.CoordinateTransformer;
...
public class GisCoordinateTransformation extends BaseTransform<GisCoordinateTransformationMeta, GisCoordinateTransformationData> 
implements ITransform<GisCoordinateTransformationMeta, GisCoordinateTransformationData> {
    ...
    public GisCoordinateTransformation(TransformMeta s, GisCoordinateTransformationMeta meta, GisCoordinateTransformationData data, int c, PipelineMeta t, Pipeline dis) {
        ...
        if (first) {
            ...
            if (crsOperationType.equalsIgnoreCase("REPROJECT")) {
                ...
                if (!meta.isCrsFromGeometry()) {
                    transformation = getTransformation(meta.getInputCRSAuthority() + ":" + resolve(meta.getInputCRSCode()), meta.getOutputCRSAuthority() + ":"
                            + resolve(meta.getOutputCRSCode()));
                }
            ...
            if (meta.isCrsFromGeometry()) {
                ...
                if (!GeometryUtils.isNullOrEmptyGeometry(inGeometry)) {
                    ...
                    if (inGeometry.getSRID() > 0) {
                        ...
                        transformation = getTransformation("EPSG:" + inGeometry.getSRID(), meta.getOutputCRSAuthority() + ":" + resolve(meta.getOutputCRSCode()));

                    } else {
                        throw new HopException("Transformation error : Unknown SRID for geometry " + inGeometry.toString());
                    }
