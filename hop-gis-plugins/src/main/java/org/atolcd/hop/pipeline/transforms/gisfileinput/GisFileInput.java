package org.atolcd.hop.pipeline.transforms.gisfileinput;
...
import org.atolcd.hop.gis.io.AbstractFileReader;
import org.atolcd.hop.gis.io.DXFReader;
import org.atolcd.hop.gis.io.GPXReader;
import org.atolcd.hop.gis.io.GeoJSONReader;
import org.atolcd.hop.gis.io.GeoPackageReader;
import org.atolcd.hop.gis.io.MapInfoReader;
import org.atolcd.hop.gis.io.ShapefileReader;
import org.atolcd.hop.gis.io.SpatialiteReader;
import org.atolcd.hop.gis.io.features.Feature;
import org.atolcd.hop.gis.io.features.FeatureConverter;
...
implements ITransform<GisFileInputMeta,GisFileInputData>
{
    ...
    public GisFileInput(TransformMeta s, GisFileInputMeta meta, GisFileInputData data, int c, PipelineMeta t, Pipeline dis) {
    ...
    public boolean processRow() throws HopException {
        ...
        if (first) {
            ...
            if (meta.getInputFormat().equalsIgnoreCase("ESRI_SHP")) {
                fileReader = new ShapefileReader(resolve(meta.getInputFileName()), resolve(meta.getGeometryFieldName()), meta.getEncoding());
            } else if (meta.getInputFormat().equalsIgnoreCase("GEOJSON")) {
                fileReader = new GeoJSONReader(resolve(meta.getInputFileName()), resolve(meta.getGeometryFieldName()), meta.getEncoding());
            } else if (meta.getInputFormat().equalsIgnoreCase("MAPINFO_MIF")) {
                fileReader = new MapInfoReader(resolve(meta.getInputFileName()), resolve(meta.getGeometryFieldName()), meta.getEncoding());
                ...
                fileReader = new SpatialiteReader(resolve(meta.getInputFileName()), tableName, meta.getEncoding());
                ...
                String ellipseAsPolygon = resolve((String) meta.getInputParameterValue("ELLIPSE_AS_POLYGON"));
            	...
            	fileReader = new GPXReader(resolve(meta.getInputFileName()),
            		resolve(meta.getGeometryFieldName()),
            		meta.getEncoding()
            	);
            ...
            String forceToMultigeometry = resolve((String) meta.getInputParameterValue("FORCE_TO_MULTIGEOMETRY"));
