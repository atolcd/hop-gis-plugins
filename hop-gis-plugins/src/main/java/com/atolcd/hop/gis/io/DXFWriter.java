...
import com.atolcd.hop.gis.io.features.Value;
...
public class DXFWriter extends AbstractFileWriter {
  ...
  public void writeFeatures(List<Feature> features) throws HopException {
    ...
    try {
      ...
      throw new HopException("Error writing features to " + this.dxfFileName, e);
    } catch (Exception e) {
      throw new HopException("Error converting to Entity " + this.dxfFileName, e);
    }
  }
  ...
  private Entity toEntity(Feature feature, String id, String text) throws Exception {
    ...
    Entity entity = new Entity(id, geometry, "", text);
        ...
        if (!value.getField().getType().equals(FieldType.GEOMETRY)
          ...
          if (classz != null) {
            ...
            entity.AddExtendedData(
                value.getField().getName(), String.class, String.valueOf(value.getValue()));
          }
        }
      }
    }
