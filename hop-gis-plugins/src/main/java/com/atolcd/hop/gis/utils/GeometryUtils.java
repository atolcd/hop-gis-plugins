...
public final class GeometryUtils {
  ...
  /**
   * Returns the EWKT string geometry with SRID and Z coordinates (if possible)
   *
   * @param geometry
   * @param forceTo2d
   * @return
  ...
  public static Geometry get2DGeometry(Geometry geometry) {
    ...
    if (!isNullOrEmptyGeometry(geometry)) {
      ...
      if (geometry instanceof Point) {
        ...
        outputGeometry =
            geometryFactory.createMultiPoint(get2DCoordinates(geometry.getCoordinates()));

        // Linestring
      } else if (geometry instanceof LineString) {

        outputGeometry =
            geometryFactory.createLineString(get2DCoordinates(geometry.getCoordinates()));
        ...
        for (int i = 0; i < polygon.getNumInteriorRing(); i++) {
          interiorRings[i] =
              geometryFactory.createLinearRing(
                  get2DGeometry(polygon.getInteriorRingN(i)).getCoordinates());
        }
  ...
  public static Geometry getGeometryFromType(Geometry geometry, Class<?> geometryClass) {
    ...
    for (Geometry typedGeometry : typedGeometries) {
      ...
      if (typedGeometry instanceof MultiLineString
          || typedGeometry instanceof MultiPoint
          || typedGeometry instanceof MultiPolygon) {
      ...
      if (geometry instanceof Point) {
        ...
        outputGeometry =
            geometryFactory.createMultiLineString(
                GeometryFactory.toLineStringArray(lineStringList));
        ...
        outputGeometry =
            geometryFactory.createMultiPolygon(GeometryFactory.toPolygonArray(polygonList));
   ...
   * @return
   */
  public static HashMap<String, Object> getGeometryCollectionInfos(
