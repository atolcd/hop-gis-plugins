...
public class ShapefileReader extends AbstractFileReader {
  ...
  public ShapefileReader(String fileName, String geometryFieldName, String charsetName)
    ...
    try {
      ...
      this.dbfFileExist =
          new File(checkFilename(replaceFileExtension(fileName, ".shp", ".dbf")).getFile())
              .exists();
      ...
      // Entête shapefile
      EndianDataInputStream endianInputStream =
          new EndianDataInputStream(new FileInputStream(this.shpFileName));
      ShapefileHeader shapeFileheader = new ShapefileHeader(endianInputStream);

      // vérification du type
      if (!(shapeFileheader.getShapeType() == 11
          || shapeFileheader.getShapeType() == 13
          || shapeFileheader.getShapeType() == 15
          || shapeFileheader.getShapeType() == 18
          || shapeFileheader.getShapeType() == 1
          || shapeFileheader.getShapeType() == 3
          || shapeFileheader.getShapeType() == 5
          || shapeFileheader.getShapeType() == 8
          || shapeFileheader.getShapeType() == 21
          || shapeFileheader.getShapeType() == 23
          || shapeFileheader.getShapeType() == 25
          || shapeFileheader.getShapeType() == 28)) {

        throw new HopException(
            "Shapefile type "
                + Shapefile.getShapeTypeDescription(shapeFileheader.getShapeType())
                + " not supported");
      }
