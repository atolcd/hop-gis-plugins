...
public class GeoPackageReader extends AbstractFileReader {
	...
	private static SimpleDateFormat dateComplexFormat = new SimpleDateFormat("EEE MMM d HH:mm:ss zzz yyyy");
    ...
    @SuppressWarnings("rawtypes")
	public GeoPackageReader(String fileName, String tableName, String geometryFieldName, String charsetName) throws HopException {
        ...
        knownPatterns.add(dateTimeFormat);
        knownPatterns.add(dateComplexFormat);
    	...
    	//Sinon, table "features" ou "attributes"
        	...
        	if(!this.gpkgTables.containsKey(this.layerName.toUpperCase())){
        		throw new HopException("Error initialize reader : Table " + this.layerName + " not found");
        	}
    	    ...
    	    for(int i = 0; i < table.columnCount(); i++){
    	    	...
    	    	if (column.getDataType() == null) {
    	    		...
    	    		if(table instanceof FeatureTable && column.getIndex() == ((FeatureTable)table).getGeometryColumnIndex()){
    	    			...
    	    			throw new HopException("Error initialize reader : Unknow data type for column " + column.getName());
    	    		}  
    	    		...
    	    		throw new HopException("Error initialize reader : Type of column " + column.getType() + " not allowed for attribut table " + this.layerName);
	...
	public List<Feature> getFeatures() throws HopException {
    	...
    	if(this.gpkgContents){
			...
			try {
				...
				throw new HopException("Error reading features :" + e.getMessage());
			}
        		...
        		for(Field field : this.fields){
        			...
        			if(inValue != null){
	        			...
	        			//date
	        					...
	        					outValue = tryToParseDate(inValue);
    ...
    public Object tryToParseDate(Object inValue) {
    	for(SimpleDateFormat pattern : knownPatterns) {
    		try {
    			return pattern.parse(inValue.toString());
    		}
	 ...
	 */
	public static org.locationtech.jts.geom.Geometry toJtsGeometry(mil.nga.sf.Geometry geometry) throws HopException{
		...
		if(geometry != null){
			...
			//GeometryCollection	
				...
				List<org.locationtech.jts.geom.Geometry> jtsGeometries = new ArrayList<org.locationtech.jts.geom.Geometry>();
				...
				return geometryFactory.createGeometryCollection(jtsGeometries.toArray(new org.locationtech.jts.geom.Geometry[jtsGeometries.size()]));
				
			}else{
				
				throw new HopException(geometry.getGeometryType().getName() + " is not supported.");
	 ...
	 * @return
	 */
	private static org.locationtech.jts.geom.Coordinate toJtsCoordinate(mil.nga.sf.Point point) {
		
		org.locationtech.jts.geom.Coordinate coordinate = new org.locationtech.jts.geom.Coordinate(point.getX(), point.getY());
		if(point.getZ() != null){
	...
	private static org.locationtech.jts.geom.Coordinate[] toJtsCoordinates(List<mil.nga.sf.Point> points) {
		
		org.locationtech.jts.geom.CoordinateList coordinateList = new org.locationtech.jts.geom.CoordinateList();
	 ...
	 * @return
	 */
	private static org.locationtech.jts.geom.MultiPoint toJtsMultiPoint(mil.nga.sf.MultiPoint multiPoint) {
	 ...
	 * @return
	 */
	private static org.locationtech.jts.geom.LineString toJtsLineString(mil.nga.sf.LineString lineString) {
	 ...
	 * @return
	 */
	private static org.locationtech.jts.geom.MultiLineString toJtsMultiLineString(mil.nga.sf.MultiLineString multiLineString) {

		List<org.locationtech.jts.geom.LineString> lineStrings = new ArrayList<org.locationtech.jts.geom.LineString>() ;
		...
		return geometryFactory.createMultiLineString(lineStrings.toArray(new org.locationtech.jts.geom.LineString[lineStrings.size()]));
	...
	private static org.locationtech.jts.geom.Polygon toJtsPolygon(mil.nga.sf.Polygon polygon) {
		...
		org.locationtech.jts.geom.LinearRing exteriorRing = null;
		List<org.locationtech.jts.geom.LinearRing> interiorRings = new ArrayList<org.locationtech.jts.geom.LinearRing>() ;
		
		for(mil.nga.sf.LineString ring : ((mil.nga.sf.Polygon) polygon).getRings()){
			
			org.locationtech.jts.geom.LinearRing linearRing = geometryFactory.createLinearRing(toJtsCoordinates(ring.getPoints()));
			org.locationtech.jts.geom.Envelope jtsEnvelope = linearRing.getEnvelopeInternal();
		...
		return  geometryFactory.createPolygon(exteriorRing, interiorRings.toArray(new org.locationtech.jts.geom.LinearRing[interiorRings.size()]));
	 ...
	 * @return
	 */
	private static org.locationtech.jts.geom.MultiPolygon toJtsMultiPolygon(mil.nga.sf.MultiPolygon multiPolygon) {
		...
		return geometryFactory.createMultiPolygon(polygons.toArray(new org.locationtech.jts.geom.Polygon[polygons.size()]));
