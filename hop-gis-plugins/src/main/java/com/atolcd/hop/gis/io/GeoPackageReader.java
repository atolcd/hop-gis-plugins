...
public class GeoPackageReader extends AbstractFileReader {
  ...
  public GeoPackageReader(
      String fileName, String tableName, String geometryFieldName, String charsetName)
      throws HopException {
      ...
      for (int i = 0; i < table.columnCount(); i++) {
        ...
        if (column.getDataType() == null) {
          ...
          if (table instanceof FeatureTable
              && column.getIndex() == ((FeatureTable) table).getGeometryColumnIndex()) {

            field = new Field(this.geometryFieldName, FieldType.GEOMETRY, null, null);

          } else {
            throw new HopException(
                "Error initialize reader : Unknow data type for column " + column.getName());
          }
          ...
          throw new HopException(
              "Error initialize reader : Type of column "
                  + column.getType()
                  + " not allowed for attribut table "
                  + this.layerName);
        }
  ...
  public List<Feature> getFeatures() throws HopException {
    ...
    if (this.gpkgContents) {
      ...
      if (this.limit <= 0) {
        ...
        userCoreResult =
            userDao.query(null, null, null, null, null, null, String.valueOf(this.limit));
      }
   ...
   */
  public static org.locationtech.jts.geom.Geometry toJtsGeometry(mil.nga.sf.Geometry geometry)
      throws HopException {
    ...
    if (geometry != null) {
      ...
      if (geometry.getGeometryType().equals(mil.nga.sf.GeometryType.POINT)) {
        ...
        List<org.locationtech.jts.geom.Geometry> jtsGeometries =
            new ArrayList<org.locationtech.jts.geom.Geometry>();
        for (mil.nga.sf.Geometry gpkgSubgeometry :
            ((mil.nga.sf.GeometryCollection<?>) geometry).getGeometries()) {
          jtsGeometries.add(toJtsGeometry(gpkgSubgeometry));
        }

        return geometryFactory.createGeometryCollection(
            jtsGeometries.toArray(new org.locationtech.jts.geom.Geometry[jtsGeometries.size()]));
  ...
  private static org.locationtech.jts.geom.Coordinate[] toJtsCoordinates(
      List<mil.nga.sf.Point> points) {

    org.locationtech.jts.geom.CoordinateList coordinateList =
        new org.locationtech.jts.geom.CoordinateList();
   ...
   * @return
   */
  private static org.locationtech.jts.geom.MultiPoint toJtsMultiPoint(
      mil.nga.sf.MultiPoint multiPoint) {
   ...
   * @return
   */
  private static org.locationtech.jts.geom.LineString toJtsLineString(
      mil.nga.sf.LineString lineString) {
   ...
   * @return
   */
  private static org.locationtech.jts.geom.MultiLineString toJtsMultiLineString(
    ...
    return geometryFactory.createMultiLineString(
        lineStrings.toArray(new org.locationtech.jts.geom.LineString[lineStrings.size()]));
  }
  ...
  private static org.locationtech.jts.geom.Polygon toJtsPolygon(mil.nga.sf.Polygon polygon) {
    ...
    mil.nga.sf.GeometryEnvelope envelope =
        mil.nga.sf.util.GeometryEnvelopeBuilder.buildEnvelope(polygon);
    ...
    return geometryFactory.createPolygon(
        exteriorRing,
        interiorRings.toArray(new org.locationtech.jts.geom.LinearRing[interiorRings.size()]));
  }
   ...
   * @return
   */
  private static org.locationtech.jts.geom.MultiPolygon toJtsMultiPolygon(
      mil.nga.sf.MultiPolygon multiPolygon) {
