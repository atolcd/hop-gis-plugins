...
public class GpxReader extends AbstractReaderWriter {
  ...
  public Document read(String filename, String charsetName)
    ...
    org.jdom2.Document document =
        sxb.build(new InputStreamReader(new FileInputStream(filename), charsetName));
    ...
    if (parentMetadataElt != null) {
      ...
      metadata.setDescription(
          (String) getChildValue(parentMetadataElt, GPX_TAG_DESC, gpxNameSpace));
      metadata.setTime(
          (GregorianCalendar) getChildValue(parentMetadataElt, GPX_TAG_TIME, gpxNameSpace));
      ...
      if (authorElt != null) {
        ...
        if (gpxDocument.getVersion().equals(Document.GPX_VERSION_1_0)) {
          ...
          if (authorEmailElt != null) {
            authorEmail =
                authorEmailElt.getAttributeValue(GPX_TAG_EMAIL_ATT_ID)
                    + "@"
      ...
      metadata.setKeywords(
          (String) getChildValue(parentMetadataElt, GPX_TAG_KEYWORDS, gpxNameSpace));
    ...
    if (childElt != null && childElt.getText() != null) {
      ...
      try {
        ...
        XMLGregorianCalendar xmlCalendar =
            DatatypeFactory.newInstance().newXMLGregorianCalendar(chiltText);
