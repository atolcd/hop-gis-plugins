...
public class ParametricPlane {
   ...
   * The y direction is generated with the cross product of the normal with the x direction.
   *
   * @param basePoint
   * @param b
   * @param normal
  ...
  public ParametricPlane(Point basePoint, Point b, Vector normal) {
    this(
        ...
        MathUtils.normalize(
            MathUtils.crossProduct(normal, MathUtils.normalize(MathUtils.getVector(basePoint, b)))),
        normal);
  }
  ...
  public double[] getParameter(Point p) {
    double u = 0.0;
    double v =
        (this.directionX.getY() * this.directionY.getX())
            - (this.directionX.getX() * this.directionY.getY());

    if (v != 0.0) {
      v =
          ((p.getY() * this.directionY.getX())
                  - (this.base.getY() * this.directionY.getX())
                  - (this.directionY.getY() * p.getX())
                  + (this.base.getX() * this.directionY.getY()))
              / v;
    }
  ...
  public boolean isOnPlane(Point p) {
    ...
    double v =
        this.base.getZ() + (this.directionX.getZ() * para[0]) + (this.directionY.getZ() * para[1]);
