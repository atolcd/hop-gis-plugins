...
public class MathUtils {
  ...
  public static double absoluteValue(Vector v) {
    double r = Math.sqrt(Math.pow(v.getX(), 2) + Math.pow(v.getY(), 2) + Math.pow(v.getZ(), 2));
  ...
  public static Point rotatePointXY(Point p, Point center, double angle) {
    Point r = new Point();
    r.setX(
        (center.getX() + (Math.cos(angle) * (p.getX() - center.getX())))
            - ((p.getY() - center.getY()) * Math.sin(angle)));
    r.setY(
        (center.getY() + (Math.cos(angle) * (p.getY() - center.getY())))
