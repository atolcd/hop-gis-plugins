  ...
  private Entity addExtendedData(Entity entity, DXFEntity dxfEntity) {
    ...
    for (DXFExtendedData dxfExtendedData : dxfEntity.getExtendedData()) {
      ...
      entity.AddExtendedData(
          dxfExtendedData.getName(), dxfExtendedData.getType(), dxfExtendedData.getValue());
    }
        ...
        Entity entity =
            new Entity(
                ...
                geometryFactory.createPoint(
                    new Coordinate(
                        dxfInsertPoint.getX(), dxfInsertPoint.getY(), dxfInsertPoint.getZ())),
                Entity.TYPE_POINT,
                null);
    ...
    if (dxfpolylines != null) {
      ...
      for (DXFPolyline dxfPolyline : dxfpolylines) {
        ...
        if (((LineString) geometry).isClosed()
            && geometry.getCoordinates().length >= 4
            && this.polylineAsPolygon) {
          geometry = geometryFactory.createPolygon(geometry.getCoordinates());
        }

        Entity entity = new Entity(dxfPolyline.getID(), geometry, Entity.TYPE_POLYLINE, null);
  ...
  private List<Entity> getLWPolylines(DXFLayer layer) {
    ...
    if (dxfLwPolylines != null) {
      ...
      for (DXFPolyline dxfLwPolyline : dxfLwPolylines) {
        ...
        Entity entity = new Entity(dxfLwPolyline.getID(), geometry, Entity.TYPE_LWPOLYLINE, null);
  ...
  private List<Entity> getSplines(DXFLayer layer) {
    ...
    if (dxfSplines != null) {
      ...
      for (DXFSpline dxfSpline : dxfSplines) {
        ...
        Entity entity = new Entity(dxfSpline.getID(), geometry, Entity.TYPE_LWPOLYLINE, null);
    ...
    if (dxfCircles != null) {
      ...
      for (DXFCircle dxfCircle : dxfCircles) {
        ...
        if (!this.circleAsPolygon) {
          geometry =
              geometryFactory.createLineString(
                  ((Polygon) geometry).getExteriorRing().getCoordinates());
        }

        Entity entity = new Entity(dxfCircle.getID(), geometry, Entity.TYPE_CIRCLE, null);
  ...
  private List<Entity> getEllipses(DXFLayer layer) {
    ...
    if (dxfEllipses != null) {
      ...
      for (DXFEllipse dxfEllipse : dxfEllipses) {
        ...
        Entity entity = new Entity(dxfEllipse.getID(), geometry, Entity.TYPE_ELLIPSE, null);
  ...
  private List<Entity> getArcs(DXFLayer layer) {
    ...
    if (dxfArcs != null) {
      ...
      for (DXFArc dxfArc : dxfArcs) {
        ...
        geometry =
            geometryFactory.createLineString(setZ(geometry.getCoordinates(), dxfCenter.getZ()));

        Entity entity = new Entity(dxfArc.getID(), geometry, Entity.TYPE_ARC, null);
