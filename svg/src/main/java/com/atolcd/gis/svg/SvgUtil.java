...
public class SvgUtil {
  ...
  public static AffineTransformation getGeometryToSvgTransformation(
      Envelope geometryExtent, Envelope svgExtent) {
    ...
    AffineTransformationBuilder AffineTransformationBuilder =
        new AffineTransformationBuilder(
  ...
  private static double[] round(double[] values, int decimalCount) {
    ...
    for (int i = 0; i < values.length; i++) {
      newValues[i] =
          BigDecimal.valueOf(values[i]).setScale(decimalCount, RoundingMode.HALF_UP).doubleValue();
    }
    return newValues;
  }
}
