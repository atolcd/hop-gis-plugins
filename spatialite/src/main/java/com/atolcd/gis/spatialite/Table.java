...
public class Table {
  ...
  protected String getSqlInsertStatement() {
    ...
    String statement =
        "INSERT INTO " + name + " (" + StringUtils.join(getFieldNames(), ", ") + ") " + "VALUES(";
  ...
  protected List<String> getSqlDropTableStatements() {
    ...
    if (isSpatial()) {
      statements.add(
          "DELETE FROM "
              + Database.SQL_META_COL_GC_TABLENAME
              + " WHERE "
              + Database.SQL_META_COL_GC_TABLENAME
              + " = '"
              + name.toLowerCase()
  ...
  protected List<String> getSqlCreateTableStatements(String pkFieldName) {
    ...
    for (Field field : this.getFields()) {
      ...
      if (field.getName().equalsIgnoreCase(pkFieldName)) {
        createtableSql =
            createtableSql + field.getName() + " " + field.getTypeAffinity() + " PRIMARY KEY, ";
      } else {
      ...
      if (field.isSpatial()) {
        ...
        geometryColumnStatements.add(
            "INSERT INTO "
                + Database.SQL_TABLE_CG
                + " ("
                + Database.SQL_META_COL_GC_TABLENAME
                + ", "
                + Database.SQL_META_COL_GC_COLUMN
                + ", "
                + Database.SQL_META_COL_GC_TYPE
                + ", "
                + Database.SQL_META_COL_GC_DIM
                + ", "
                + Database.SQL_META_COL_GC_SRID
                + ", "
                + Database.SQL_META_COL_GC_INDEX
                + ") VALUES ("
                ...
                + "', "
                + geometryProperties.getGeometryType()
                + ", "
                + geometryProperties.getCoordDimension()
                + ", "
                + geometryProperties.getSrid()
                + ", "
                + 0
                + ");");
      }
