...
public class PolygonHandler implements ShapeHandler {
  ...
  public Geometry read(
    ...
    if (shapeType == 0) {
      ...
      for (int i = 0; i < holesWithoutShells.size(); i++) {
        polygons[shells.size() + i] =
            geometryFactory.createPolygon((LinearRing) holesWithoutShells.get(i), null);
      ...
      if (badRings.size() > 0) {
        geom.setUserData(
            geometryFactory.createMultiLineString(badRings.toArray(new LineString[0])));
      }
      holesForShells = null;
  ...
  /** Finds a object in a list using == instead of equals. Should be much faster than indexof */
  ...
  public void write(Geometry geometry, EndianDataOutputStream file) throws IOException {
    ...
    if (geometry instanceof MultiPolygon) {
      ...
      multi =
          new MultiPolygon(
              new Polygon[] {(Polygon) geometry}, geometry.getPrecisionModel(), geometry.getSRID());
    }
    ...
    if (myShapeType == 15) { // z
      ...
      for (int t = 0; t < npoints; t++) {
        double z = coords[t].z;
        if (Double.isNaN(z)) file.writeDoubleLE(0.0);
        else file.writeDoubleLE(z);
      }
    }
