...
public class DbfFileWriter implements DbfConsts {
  ...
  public DbfFileWriter(String file) throws IOException {
    if (DEBUG)
      System.out.println(
          "---->uk.ac.leeds.ccg.dbffile.DbfFileWriter constructed. Will identify itself as " + DBC);
  ...
  public void writeRecord(Vector rec) throws DbfFileException, IOException {
    ...
    if (rec.size() != NoFields)
      throw new DbfFileException(
          DBC + "wrong number of fields " + rec.size() + " expected " + NoFields);
    String s;
    ls.writeByteLE(' ');
    int len;
    StringBuffer tmps;
