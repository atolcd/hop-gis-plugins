 ...
 *
 * <p>Construct it with a URL or File (including the .dbf) this causes the header and field
 * definitions to be read.
 *
 * <p>Later queries return rows or columns of the database.
 *
 * <p>If a URL is specified then the whole file is read into memory<br>
 * if a file is specified then a randomAccess system is used.<br>
 * <hr>
 *
 * @author <a href="mailto:ian@geog.leeds.ac.uk">Ian Turton</a> Centre for Computaional Geography,
 *     University of Leeds, LS2 9JT, 1998. <br>
...
public class Dbf implements DbfConsts {
  ...
  public Dbf(URL url) throws java.io.IOException, DbfFileException {
    if (DEBUG)
      System.out.println(
          "---->uk.ac.leeds.ccg.dbffile.Dbf constructed. Will identify itself as " + DBC);
  ...
  public StringBuffer getFieldName(int col) {
    if (col >= numfields)
      throw new IllegalArgumentException(
          DBC
              + "column number specified is invalid. It's higher than the amount of columns available "
              + numfields);
  ...
  /**
   * gets the next record and returns it as a string. This method works on a sequential stream and
   * can not go backwards. Only useful if you want to read the whole file in one.
   *
  ...
  private StringBuffer[] GrabFile() throws java.io.IOException {
    ...
    for (int i = 0; i < last_rec; i++) records[i] = GrabNextDbfRec();
    return records;
  }
  ...
  /**
   * fetches the <i>row</i>th row of the file and parses it into an vector of objects.
   *
   * @param row - the row to fetch
   * @exception java.io.IOException on read error.
   */
  ...
  public Vector ParseRecord(StringBuffer rec) {
    ...
    for (int i = 0; i < numfields; i++) {
      ...
      switch (fielddef[i].fieldtype) {
        case 'C':
          record.addElement(
              t.substring(fielddef[i].fieldstart, fielddef[i].fieldstart + fielddef[i].fieldlen));
          break;
        case 'N':
        case 'F':
  ...
  public Integer[] getIntegerCol(int col, int start, int end)
    ...
    int k = 0, i = 0;
    if (col >= numfields) throw new DbfFileException(DBC + "No Such Column in file: " + col);
    if (fielddef[col].fieldtype != 'N')
      throw new DbfFileException(
          DBC + "Column " + col + " is not Integer " + fielddef[col].fieldtype);
    if (start < 0) throw new DbfFileException(DBC + "Start must be >= 0");
    if (end > last_rec) throw new DbfFileException(DBC + "End must be <= " + last_rec);
  ...
  public Float[] getFloatCol(int col, int start, int end)
    ...
    int k = 0, i = 0;
    if (col >= numfields) throw new DbfFileException("Dbf->No Such Column in file: " + col);
    ...
    if (start < 0) throw new DbfFileException("Dbf->Start must be >= 0");
    if (end > last_rec) throw new DbfFileException("Dbf->End must be <= " + last_rec);
    // move to start of data
    try {
      for (i = start; i < end; i++) {
        sb.setLength(0);
  ...
  public String[] getStringCol(int col, int start, int end)
    ...
    try {
      for (i = start; i < end; i++) {
        ...
        column[i - start] =
            new String(record.getBytes(), fielddef[col].fieldstart, fielddef[col].fieldlen).trim();
      }
    } catch (java.io.EOFException e) {
      System.err.println("Dbf->" + e);
      System.err.println("Dbf->record " + i + " byte " + k + " file pos ");
