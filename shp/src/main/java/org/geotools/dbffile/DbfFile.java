 ...
 *
 * <p>Construct it with a filename (including the .dbf) this causes the header and field definitions
 * to be read.
 *
 * <p>Later queries return rows or columns of the database. <hr>
 *
 * @author <a href="mailto:ian@geog.leeds.ac.uk">Ian Turton</a> Centre for Computaional Geography,
 *     University of Leeds, LS2 9JT, 1998.
 */
...
public class DbfFile implements DbfConsts {
  ...
  /**
   * For compatibilty reasons, this method is a wrapper to the new with Charset functions.
   *
   * @param file
  ...
  public Object ParseRecordColumn(byte[] rec, int wantedCol) // [sstein
    ...
    switch (fielddef[wantedCol].fieldtype) {
      ...
      case 'N': // numeric
        ...
        boolean isInteger =
            fielddef[wantedCol].fieldnumdec == 0 && fielddef[wantedCol].fieldtype == 'N';
  ...
  public Double[] getFloatCol(int col, int start, int end)
    ...
    if (fielddef[col].fieldtype != 'F') {
      throw new DbfFileException(
          "DbFi>Column " + col + " is not Double " + fielddef[col].fieldtype);
    }
